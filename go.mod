module github.com/RiemaLabs/indexer-light

go 1.22.1

replace github.com/RiemaLabs/indexer-committee => ../indexer-committee

require github.com/RiemaLabs/indexer-committee v0.0.0-202202171455

require (
	github.com/bits-and-blooms/bitset v1.7.0 // indirect
	github.com/btcsuite/btcd/btcutil v1.1.5 // indirect
	github.com/consensys/bavard v0.1.13 // indirect
	github.com/consensys/gnark-crypto v0.12.1 // indirect
	github.com/crate-crypto/go-ipa v0.0.0-20231025140028-3c0104f4b233 // indirect
	github.com/ethereum/go-verkle v0.1.1-0.20240119133216-f8289fc59149 // indirect
	github.com/holiman/uint256 v1.2.4 // indirect
	github.com/jackc/pgpassfile v1.0.0 // indirect
	github.com/jackc/pgservicefile v0.0.0-20221227161230-091c0ba34f0a // indirect
	github.com/jackc/pgx/v5 v5.4.3 // indirect
	github.com/jinzhu/inflection v1.0.0 // indirect
	github.com/jinzhu/now v1.1.5 // indirect
	github.com/mmcloughlin/addchain v0.4.0 // indirect
	golang.org/x/crypto v0.14.0 // indirect
	golang.org/x/sync v0.1.0 // indirect
	golang.org/x/sys v0.13.0 // indirect
	golang.org/x/text v0.13.0 // indirect
	gorm.io/driver/postgres v1.5.6 // indirect
	gorm.io/gorm v1.25.7-0.20240204074919-46816ad31dde // indirect
	rsc.io/tmplfunc v0.0.3 // indirect
)
